...
METADATA_REGEXS = {
    "title": (re.compile(rf"(title):\s+(.+)\n"), str),
    "date": (re.compile(rf"(date):\s+(.+)\n"), str),
}
...
class GeminiConvert(PostConvert):
    ...
    def parse(self):
        ...
        html_blocks = re.findall(GeminiConvert.HTML_RE, str(self.dest))
...
class GopherConvert(PostConvert):
    ...
    def http_menu(
        title: str, url: str, hostpage: str = DEFAULT_HOSTPAGE, port: int = DEFAULT_PORT
    ) -> str:
        """
        ...
        return (
            "[h|" + title + "|" + "URL:" + url + "|" + hostpage + "|" + str(port) + "]"
        )
    ...
    def __process_redirects(self):
        ...
        redirects = re.findall(GopherConvert.HTTP_SOURCE_RE, self.dest)

        for title, url in redirects:
            title_dest = title.replace("*", "")[1:-1]
            url_dest = url[1:-1]

            http_menu = GopherConvert.http_menu(title_dest, url_dest)

            redirect = title + url
            self.dest = self.dest.replace(redirect, "\n" + http_menu + "\n")
    ...
    def __process_p_tags(self):
        ...
        matchs = re.findall(GopherConvert.P_RE, self.dest, re.MULTILINE | re.DOTALL)

        for match in matchs:
            self.dest = self.dest.replace(match, "")
    ...
    def parse(self):
        ...
        self.dest = BeautifulSoup(self.dest, features="html.parser").get_text()
...
TO_CONVERT = {Document.GOPHER: GopherConvert, Document.GEMINI: GeminiConvert}
    ...
    def dump_to_files(self, dest_dir: str):
        ...
        for root, _, files in os.walk(self.__dir):
            for file in files:
                ...
                dest = file.replace(".md", "." + converter.document.value)

                f = open(dest_dir + "/" + dest, "w+")
...
def main():
    ...
    available_exts = tuple(map(lambda x: x.value, list(Document)))
